...
files.forEach((file) => {
  ...
  try {
    ...
    // Revert @classpoint/db imports back to @prisma/client
    content = content.replace(/from '@classpoint\/db'/g, "from '@prisma/client'");

    if (content !== originalContent) {
      fs.writeFileSync(filePath, content, 'utf8');
      const count = (originalContent.match(/from '@classpoint\/db'/g) || []).length;
      totalReplacements += count;
      totalFiles++;
      console.log(`✅ Reverted ${file} (${count} replacement${count > 1 ? 's' : ''})`);
    }
  } catch (error) {
...
console.log(`\n✨ Done! Reverted ${totalFiles} files with ${totalReplacements} total replacements.`);
