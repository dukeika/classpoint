...
echo "=== DEPLOYMENT ATTEMPT 5 ==="
echo "Commit: $NEW_COMMIT (Fix: Use Next.js standalone server.js)"
echo ""
...
for i in {1..20}; do
  sleep 5
  status=$(aws apprunner list-services --region $REGION --profile $PROFILE --query "ServiceSummaryList[?ServiceName=='classpoint-web'].Status" --output text)
  if [ -z "$status" ]; then
